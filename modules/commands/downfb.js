...
async function getData(url) {
  try {
    ...
    if (!k_exp || !k_token) throw new Error('Không thể lấy token.');
...
this.handleEvent = async function({ api, event }) {    
    ...
    for (const str of urls) {
        if (/facebook\.com\//.test(str)) {
            // Thả biểu tượng đồng hồ cát khi bắt đầu xử lý
            api.setMessageReaction("⏳", event.messageID, (err) => {
                if (err) console.error("Lỗi khi thả reaction ⏳:", err);
            }, true);

            const res = await getData(str);
            if (res && res.url) {
                let attachment = await streamURL(res.url, 'mp4');
                send({ body: `${head('FACEBOOK')}`, attachment });
                // Thả biểu tượng ✅ khi tải thành công
                api.setMessageReaction("✅", event.messageID, (err) => {
                    if (err) console.error("Lỗi khi thả reaction ✅:", err);
                }, true);
            } else {
                // send({ body: 'Không thể tải xuống video.' });
                // Thả biểu tượng ❌ khi tải thất bại
                api.setMessageReaction("❌", event.messageID, (err) => {
                    if (err) console.error("Lỗi khi thả reaction ❌:", err);
                }, true);
            }
