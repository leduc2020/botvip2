...
//---------------END --------------<
       ...
       if (!command) {
         ...
         api.sendMessage({
          body: `-⪼ Lệnh không tồn tại\n-⪼ ${threadSetting.PREFIX || PREFIX}menu để xem các lệnh\n-⪼ Lệnh gần giống là: ${checker.bestMatch.target}\n-⪼ Uptime: ${H+$+M+$+S} `,
          attachment: global.ytb_rst.splice(0, 1) // Thêm tệp đính kèm từ global
...
if(command) {
  ...
  if (all.status == true && ADMINBOT.includes(all.author) && !NDH.includes(senderID) &&  !ADMINBOT.includes(senderID)) return send(`Vào lúc: ${all.time}\nBạn đã bị admin bot: ${name(all.author)} cấm\n👤 Liên hệ với admin để được hỗ trợ`);
  if (all.status == true && is_qtv_box(all.author) && !NDH.includes(senderID) && !ADMINBOT.includes(sid) && !is_qtv_box(senderID)) return send(`Vào lúc ${all.time}\nBạn đã bị qtv box: ${name(all.author)} cấm`);
        };
